...
def cal_measure(PROB_NAME, DATA_NAME):
    ...
    def cal_single_measure(y_list, prob, threshold):
        ...
        for i in range(it_max):
            ...
            g_list[i] = g_norm
        measure = np.where(g_list < threshold)[0]
        if len(measure) > 0:
            measure = measure[0]
        else:
            measure = -1
    ...
    measure_record = dict()
    for name in names:
        measure_record[name] = np.zeros(num_tests)
...
if __name__ == "__main__":
    ...
    # cal_measure(PROB_NAME, DATA_NAME)
